"""Production Provider adapters for Scholar discovery and document parsing."""
...
from app.jobs import JobRecord, LongTaskSubmitter, PersistentJobRepository, PersistentJobWorker
...
class AcademicBackend(Protocol):
    ...
    async def find_fulltext(self, doi: str | None = None, openalex_id: str | None = None, arxiv_id: str | None = None) -> FullTextResult: ...

    async def download_open_pdf(self, download_token: str, filename: str | None = None) -> dict[str, str | int | bool]: ...


class CandidateKnowledgeRepository:
    """External candidates are metadata snapshots, never canonical corpus data."""

    def __init__(self, project_root: Path) -> None:
        self.path = project_root.expanduser().resolve() / "data" / "candidate_knowledge" / "papers.json"
...
class AcademicLiteratureProviderAdapter:
    def __init__(self, backend: AcademicBackend, candidates: CandidateKnowledgeRepository) -> None:
        self.backend = backend
        self.candidates = candidates

    def search(self, arguments: Mapping[str, Any], context: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        return {"results": papers, "result_count": len(papers), "provider_failures": [value.to_dict() for value in result.failures]}

    def get_metadata(self, arguments: Mapping[str, Any], context: Mapping[str, Any]) -> Mapping[str, Any]:
        value = self.candidates.get(str(arguments["paper_id"]))
        return {**value, "lightweight": True}

    def resolve_publication_date(self, arguments: Mapping[str, Any], context: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        return {
            ...
            "candidates": values,
            "provider_failures": [value.to_dict() for value in failures] if isinstance(failures, list) else [],
        }
    ...
    def download(self, paper_id: str) -> Mapping[str, Any]:
        ...
        fulltext = _sync(self.backend.find_fulltext(
            doi=str(paper["doi"]) if paper.get("doi") else None,
            openalex_id=str(external_ids["openalex"]) if external_ids.get("openalex") else None,
            arxiv_id=str(paper["arxiv_id"]) if paper.get("arxiv_id") else None,
        ))
        location = next((value for value in fulltext.locations if value.download_token), None)
        if location is None or location.download_token is None:
            raise FileNotFoundError("候选论文没有可下载的开放 PDF。")
        return _sync(self.backend.download_open_pdf(location.download_token, filename=f"{paper_id}.pdf"))
...
class CanonicalDocumentParseProviderAdapter:
    def __init__(self, project_root: Path, *, parser: ParseProvider = parse_paper, indexer: Callable[[Path], Mapping[str, Any]] | None = None) -> None:
...
def build_bootstrap_provider_composition(project_root: Path, backend: AcademicBackend, *, parser: ParseProvider = parse_paper, worker_id: str = "bootstrap-worker", indexer: Callable[[Path], Mapping[str, Any]] | None = None) -> BootstrapProviderComposition:
    root = project_root.expanduser().resolve()
    repository = PersistentJobRepository(root)
    results = JobResultStore(root)
    literature = AcademicLiteratureProviderAdapter(backend, CandidateKnowledgeRepository(root))
    ...
    def download_job(record: JobRecord, context: Any) -> str:
        ...
        if downloaded_path and indexer is not None:
            parse_job = submitter.handler("document.parse")(
                {"path": downloaded_path, "mode": "formal"},
                {"workspace_id": record.workspace_id, "scope_version": record.scope_version},
            )
    ...
    def parse_job(record: JobRecord, context: Any) -> str:
        context.raise_if_cancelled()
        result = document_parser.parse(str(record.payload["path"]), str(record.payload.get("mode") or "formal"))
        return results.write(record.job_id, result)

    def status(arguments: Mapping[str, Any], context: Mapping[str, Any]) -> Mapping[str, Any]:
        record = repository.get(str(arguments["job_id"]))
        result = results.read(record.result_reference) if record.status == "SUCCEEDED" and record.result_reference else None
        return {"job_id": record.job_id, "status": record.status.casefold(), "result": result, "error_type": record.error_type, "error_summary": record.error_summary}
    ...
    return BootstrapProviderComposition(
        ...
        worker=PersistentJobWorker(repository, {"literature.download": download_job, "document.parse": parse_job}, worker_id=worker_id),
