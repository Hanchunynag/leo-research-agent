...
The decision is serialized in the EvidencePack metadata and, when a
`ResearchRunHarness` is supplied, in the `FRESHNESS_DECIDE` trace step.
...
metadata/identity failures) and are recorded in pack metadata and existing
Harness tool/provider trace. They never become `UNSUPPORTED` claims. Local
...
In Phase 3B, the Deep Agent does not call a Web provider. Research Subagents
receive only the high-level `ResearchCapabilityService` adapter; the adapter
constructs `ResearchRequest`, applies this policy and records the existing
Research Harness trace. Production composition supplies the same Web adapter
